...
const int64 nChainStartTime = 1376591211; 
const int64 nTestNetStartTime = 1382904104; // 2013-08-03 18:00:00 GMT
...
const string strMessageMagic = "Ocoin Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // Ocoin: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Ocoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Ocoin
    bool fStrictPayToScriptHash = true; // Always active in Ocoin
    ...
    // ppcoin: fees are not collected by miners as in Ocoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Ocoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        txNew.strTxComment = "text:Ocoin genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0xede221767db6b172ad1a58be9256a1a3520b0ba3b2f0c2ec4495ff2095d23d64"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("OcoinMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("OcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadOcoinMiner(void* parg);

static bool fGenerateOcoins = false;
...
void OcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("Ocoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateOcoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateOcoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running OcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadOcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        OcoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadOcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadOcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadOcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateOcoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateOcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateOcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d OcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadOcoinMiner, pwallet))
                printf("Error: NewThread(ThreadOcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
